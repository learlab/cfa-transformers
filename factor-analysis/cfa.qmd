...
df <- read_csv(data_path, show_col_types = FALSE) %>% 
  ...
  mutate(across(starts_with("overall_"), round)) %>%
...
```{r}
ggplot(df, aes(x = overall_1, fill=overall_1)) +
...
```{r}
spec <- ' overall =~ overall_1 + overall_2
...
fit1.labels <- c("Rater 1", "Rater 2", "Deberta 1", "Deberta 2", "Overall")
fit1.ss <- standardizedSolution(fit1)

fit2 <- cfa(
  ' human =~ overall_1 + overall_2
    llm =~ deberta_overall_1 + deberta_overall_2
  ',
  data=df,
  std.lv=TRUE,
  ordered=TRUE,
...
fit2.labels <- c("Rater 1", "Rater 2", "Deberta 1", "Deberta 2", "Overall\n(Human)", "Overall\n(AES)")
