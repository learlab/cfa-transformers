...
```{r}
cols <- df[, c("overall_1", "overall_2", "deberta_overall_1", "deberta_overall_2")] %>% 
  mutate_all(as.numeric)

# Human raters are random, so model=twoway
hum_reliability <- icc(cols[, 1:2], model="twoway", type="consistency")$value
# All essays were rated by the same two LLMs, so model=oneway
llm_reliability <- icc(cols[, 3:4], model="oneway", type="consistency")$value

mean_human = rowMeans(cols[,1:2])
mean_llm = rowMeans(cols[,3:4])
true_corr = cor(mean_human, mean_llm, method="spearman") / sqrt(hum_reliability*llm_reliability)

cat("Human Reliabality: ", hum_reliability)
cat("\nLLM Reliability: ", llm_reliability)
cat("\nTrue Correlation: ", true_corr)
```


```{r}
get_cor <- function(x, y, corfunc) {
  x <- as.numeric(x)
  y <- as.numeric(y)
  corfunc(x, y)
  
}
irr_human = get_cor(df$overall_1, df$overall_2, cor)
irr_llm = get_cor(df$deberta_overall_1, df$deberta_overall_2, cor)

mean_human = rowMeans(cbind(df$overall_1, df$overall_2))
mean_llm = rowMeans(cbind(df$deberta_overall_1, df$deberta_overall_2))
true_corr = get_cor(mean_human, mean_llm, cor) / sqrt(irr_human*irr_llm)
```
